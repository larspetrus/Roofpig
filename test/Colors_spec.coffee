...
describe "Colors", ->
  describe "of", ->
    it "has default values", ->
      colors = new Colors(new PovTracker(), "", "", "")
    ...
    it "can change colors", ->
      colors = new Colors(new PovTracker(), "", "", "", "R:o L:#abc solved:r c:#123")
  ...
  describe "#to_draw", ->
    ...
    it "colors only specified stickers", ->
      colors = new Colors(new PovTracker(), "U*", "")
      expect(colors.to_draw('UFR', 'F')).to.deep.equal(color: colors.of('F'), hovers: true)
    ...
    it "solved overrides colored", ->
      colors = new Colors(new PovTracker(), "U*", "F*")
    ...
    it "last tweak color wins", ->
      colors = new Colors(new PovTracker(), "*", "", "R:U* L:F*")
    ...
    describe "tweaks", ->
      it "sets X and colors", ->
        colors = new Colors(new PovTracker(), "", "*", ".Xx:uFR  D.L:UfR")
      ...
      it "overrides colored and solved", ->
        colors = new Colors(new PovTracker(), "U*", "D*", "L:U  R:D")
  ...
  describe "side pov adjustments", ->
    it "side colors", ->
      plain_colors = new Colors(new PovTracker(), "", "", "")
      pov_colors = new Colors(new PovTracker(new SingleMove("S")), "", "", "")
      expect(pov_colors.of('F')).to.equal(plain_colors.of('F'))
      expect(pov_colors.of('R')).to.equal(plain_colors.of('U'))

    it "colored", ->
      colors = new Colors(new PovTracker(new SingleMove("S")), "u", "", "")
    ...
    it "solved", ->
      colors = new Colors(new PovTracker(new SingleMove("S")), "Ufr", "r", "")
    ...
    it "tweaks", ->
      untweaked = new Colors(new PovTracker(new SingleMove("S")), "", "", "")
      ...
      tweaked = new Colors(new PovTracker(new SingleMove("S")), "", "", "X:Uf RF:DL")
